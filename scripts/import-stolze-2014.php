<?php
/**
 * Import the 2014 Stolze Openair line-up.
 *
 * openairguide.net never recorded the 2014 (or 2016) edition, so this dataset
 * was recovered from the festival's own archived site (see data/stolze-2014-lineup.json).
 *
 * Usage: wp eval-file scripts/import-stolze-2014.php [dry-run|apply]
 */
...
$mode         = $args[0] ?? 'dry-run';
$dataset_file = dirname( __DIR__ ) . '/data/stolze-2014-lineup.json';
...
if ( ! is_array( $dataset ) || empty( $dataset['2014'] ) ) {
	WP_CLI::error( 'Dataset is missing the 2014 line-up.' );
}
...
$year = stolze_year_by_title( 2014 );
if ( ! $year ) {
	WP_CLI::error( 'Year post not found: 2014' );
}
...
foreach ( $dataset['2014'] as $performance ) {
	...
	WP_CLI::log( sprintf( '[2014] %s: %s%s%s', $action, $name, $time ? ' @ ' . $time : '', $stage ? ' (' . $stage . ')' : '' ) );
	if ( 'dry-run' === $mode ) {
